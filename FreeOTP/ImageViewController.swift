...
class ImageViewController : UICollectionViewController, UICollectionViewDelegateFlowLayout {
    fileprivate var sections: PHFetchResult? = nil
    fileprivate var items: PHFetchResult? = nil
    ...
    override func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let items = PHAsset.fetchAssets(with: PHAssetMediaType.image, options: nil)
        return items.count
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let a = self.items[indexPath.row] as! PHAsset
